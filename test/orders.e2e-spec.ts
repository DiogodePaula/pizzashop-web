...
test("list orders", async ({ page }) => {
  ...
  // USAR O AWAIT NOS TESTS DE VISIBLE GARANTE QUE O ERRO NAO SEJA POR O TEST CHAMAR SEM O COMPONENTE ESTAR MONTADO CASO ELE NAO ACHE ESTOURA ERRO
  await expect(
...
test("filter by order id", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Filtrar resawait ultados" }).click();
...
test("filter by customer name", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Filtrar resultawait ados" }).click();
